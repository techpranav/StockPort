...
from config.constants.Messages import (
    ...
    WORD_HEADER_FINANCIAL_METRICS,
    WORD_HEADER_TECHNICAL_ANALYSIS,
    WORD_HEADER_FUNDAMENTAL_ANALYSIS,
    WORD_HEADER_PORTFOLIO_ANALYSIS,
    WORD_HEADER_INCOME_STATEMENT,
    WORD_HEADER_BALANCE_SHEET,
    WORD_HEADER_CASH_FLOW,
    WORD_HEADER_AI_SUMMARY,
...
class ReportService:
    ...
    def generate_excel_report(self, symbol: str, data: Dict[str, Any], 
                            days_back: int = None) -> str:
        ...
        try:
            ...
            excel_path = os.path.join(self.reports_dir, 
                                    f"{REPORT_FILE_PATTERN.format(symbol=symbol, timestamp=timestamp)}{EXCEL_EXTENSION}")
            ...
            with pd.ExcelWriter(excel_path) as writer:
                ...
                if 'info' in data:
                    try:
                        ...
                        sheets_written += 1
                        DebugUtils.debug(f"Successfully wrote company info sheet for {symbol}")
                    except Exception as e:
                        DebugUtils.error(f"Error writing company info sheet for {symbol}: {str(e)}")
                ...
                if 'metrics' in data:
                    try:
                        ...
                        sheets_written += 1
                        DebugUtils.debug(f"Successfully wrote metrics sheet for {symbol}")
                    except Exception as e:
                        DebugUtils.error(f"Error writing metrics sheet for {symbol}: {str(e)}")
                ...
                if 'technical_analysis' in data:
                    try:
                        ...
                        sheets_written += 1
                        DebugUtils.debug(f"Successfully wrote technical analysis sheet for {symbol}")
                    except Exception as e:
                        DebugUtils.error(f"Error writing technical analysis sheet for {symbol}: {str(e)}")
                ...
                if 'fundamental_analysis' in data:
                    try:
                        ...
                        sheets_written += 1
                        DebugUtils.debug(f"Successfully wrote fundamental analysis sheet for {symbol}")
                    except Exception as e:
                        DebugUtils.error(f"Error writing fundamental analysis sheet for {symbol}: {str(e)}")
                ...
                if 'portfolio_analysis' in data:
                    try:
                        ...
                        sheets_written += 1
                        DebugUtils.debug(f"Successfully wrote portfolio analysis sheet for {symbol}")
                    except Exception as e:
                        DebugUtils.error(f"Error writing portfolio analysis sheet for {symbol}: {str(e)}")
                ...
                if 'financials' in data:
                    try:
                        ...
                        sheets_written += 1
                        DebugUtils.debug(f"Successfully wrote financial statements for {symbol}")
                    except Exception as e:
                        DebugUtils.error(f"Error writing financial statements for {symbol}: {str(e)}")
                
                # Ensure at least one sheet exists
                if sheets_written == 0:
                    DebugUtils.warning(f"No sheets were written for {symbol}, creating a summary sheet")
                    # Create a basic summary sheet
                    summary_data = {
                        'Symbol': [symbol],
                        'Report Generated': [datetime.now().strftime('%Y-%m-%d %H:%M:%S')],
                        'Status': ['Data analysis completed but no detailed sheets could be generated']
                    }
                    summary_df = pd.DataFrame(summary_data)
                    summary_df.to_excel(writer, sheet_name='Summary', index=False)
                    sheets_written += 1
                
                DebugUtils.info(f"Excel report generated for {symbol} with {sheets_written} sheets")
            
            # Register the report with the report manager
            report_id = self.report_manager.register_report(
                symbol=symbol,
                report_type=REPORT_TYPE_EXCEL,
            ...
            print(f"Error generating Excel report for {symbol}: {str(e)}")
            raise
    
    def generate_word_report(self, symbol: str, data: Dict[str, Any], 
                           days_back: int = None) -> str:
        """Generate Word report for stock analysis."""
        try:
            if days_back is None:
                days_back = self.days_back
            ...
            print(f"Error generating Word report for {symbol}: {str(e)}")
            raise
    
    def cleanup_old_reports(self, days: int) -> int:
        """Clean up reports older than specified days."""
        return self.report_manager.cleanup_old_reports(days)
    
    def get_reports(self, symbol: str = None, days_limit: int = None) -> List[Dict[str, Any]]:
        """Get available reports."""
        return self.report_manager.get_reports(symbol=symbol, days_limit=days_limit)
    
    def get_report_stats(self) -> Dict[str, Any]:
        """Get report statistics."""
        return self.report_manager.get_report_stats()
    ...
    def _write_company_info(self, writer: pd.ExcelWriter, info: Dict[str, Any]) -> None:
        ...
        if info is None:
            DebugUtils.warning("Company info is None, creating empty company info sheet")
            empty_df = pd.DataFrame({'Message': ['No company information available']})
            empty_df.to_excel(writer, sheet_name=SHEET_COMPANY_INFO, index=False)
            return
        ...
        try:
            ...
            for col in cleaned_df.columns:
                if cleaned_df[col].dtype in ['float64', 'int64']:
                    ...
                    cleaned_df[col] = cleaned_df[col].fillna('').infer_objects(copy=False)
    ...
    def _add_dataframe_to_doc(self, doc: Document, df: pd.DataFrame) -> None:
        ...
        header_cells[0].text = STR_METRIC
