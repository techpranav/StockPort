...
def render_user_profile():
    ...
    col1, col2 = st.columns(2)
        ...
        with col2:
        # License information - force fresh validation
    ...
    st.markdown("---")
        ...
        with col1:
                if st.button("🔄 Refresh License Status", key="refresh_license_active"):
                    ...
                    try:
                        ...
                        if has_payment_context and upgrade_plan:
                            # Auto-upgrade to Pro license
                            st.info(f"🔄 Payment detected - upgrading to {upgrade_plan.replace('_', ' ').title()}...")
                            try:
                                license_key = license_service.create_license(current_user['id'], upgrade_plan)
                                st.success(f"✅ Pro license activated! Your {upgrade_plan.replace('_', ' ').title()} plan is now active.")
        ...
        return
                        ...
                        if license_valid_new:
                            ...
                            # Clear any cached license data and payment state
